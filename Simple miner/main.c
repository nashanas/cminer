...
int cmp2 (const void * a, const void * b ) {
    int32_t s[8];
    for(int i = 0; i< 9; i++) s[i] = ( *((int*)a+i))-( *((int*)b+i));
    if(s[0]==0&&s[1]==0&&s[2]==0&&s[3]==0&&s[4]==0&&s[5]==0&&s[6]==0) return s[7];
    if(s[0]==0&&s[1]==0&&s[2]==0&&s[3]==0&&s[4]==0&&s[5]==0) return s[6];
    if(s[0]==0&&s[1]==0&&s[2]==0&&s[3]==0&&s[4]==0) return s[5];
    if(s[0]==0&&s[1]==0&&s[2]==0&&s[3]==0) return s[4];
  ...
  //определение размера длины массива Х для будущего раунда.
    ...
    for(int i = 1; i<n; i++){
        ...
        if(temp0==temp) c++;
            ...
            temp0 = temp;
            x0_length = x0_length + binomialCoeff(c, 2);
            c = 1;
        }
    }
    ...
    //раунд 1
    uint32_t *X0;
    X0 = (uint32_t*) malloc(m * x0_length * sizeof(uint32_t));
    int i=0, k=0; j=0;
    uint32_t t0, t1;
    while (k < n) {
        t0 = *(X+k*m);
...
//        printf("compare: %x and %x \n",t0, t1);
        while(t0 == t1){
//            printf("pairs: %x and %x \n",*(X+(k-1)*m+1), *(X+(k+i)*m+1));
            for(int w = 1; w<10; w++){
                *(X0 + j*m + w-1) = *(X+(k-1)*m+w) ^ *(X+(k+i)*m+w);
            }
            *(X0 + j * m + 9) = *(X+(k-1) * m+10);
            *(X0 + j * m + 10) = *(X+(k+i) * m+10);
            i++; j++;
            t1 = *(X+(k+i)*m);
        }
        i=0;
    ...
    //раунд 2
    //сортировка
    qsort(X0, x0_length, sizeof(uint32_t)*11, cmp1);
    //определение размера нового массива
    uint64_t x1_length = 0;
    temp0 =  *X0;
    c = 1; //счетчик слов, с коллизией в первых 20 битах
    for(int i = 1; i<x0_length; i++){
        uint32_t temp = *(X0+i*m);
        if(temp0==temp) c++;
        else{
            temp0 = temp;
    ...
    uint32_t *X1;
    uint16_t x1_col = 12;
    X1 = (uint32_t*) malloc(x1_col * x0_length * sizeof(uint32_t));
    i=0; k=0; j=0;
    while (k < x0_length) {
        t0 = *(X0+k*m);
        k++;
        ...
        while(t0 == t1){
            for(int w = 1; w<9; w++){
                *(X1 + j*x1_col + w-1) = *(X0+(k-1)*m+w) ^ *(X0+(k+i)*m+w);
            }
            *(X1 + j * x1_col + 8) =  *(X0 + (k - 1) * m + 9);
            *(X1 + j * x1_col + 9) =  *(X0 + (k - 1) * m + 10);
            *(X1 + j * x1_col + 10) = *(X0 + (k + i) * m + 9);
            *(X1 + j * x1_col + 11) = *(X0 + (k + i) * m + 10);
            i++; j++;
            t1 = *(X0 + (k+i)*x1_col);
        }
        i=0;
